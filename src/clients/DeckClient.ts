...
import { pathBoards, pathBoardById, pathBoardUndoDelete, pathBoardAcl, pathBoardAclById, pathStacks, pathStacksArchived, pathStacksById, pathCards, pathCardById, pathCardAssignLabel, pathCardRemoveLabel, pathCardAssignUser, pathCardUnassignUser, pathCardReorder, pathLabelById, pathLabels, pathCardAttachements, pathCardAttachementsById, pathCardRestoreAttachementsById, pathOcsUpcomingCards, pathOcsSearchCards } from "../paths/deckPath"
import { Board, CreateBoardPayload, UpdateBoardPayload, CreateAclRulePayload, AclRuleResponse, Stack, CreateStackPayload, UpdateStackPayload, CreateCardPayload, Card, UpdateCardPayload, AssignUserResponse, ReorderCardPayload, Label, CreateLabelPayload, UpdateLabelPayload, Attachment, DeckSettings, SearchedCard, OcsSearchCardQuery, OcsSearchCardsResponse, OcsUpcomingCardsRespone, UpcomingCards } from "../types/deckTypes"
import { BaseApiClient } from "../util/BaseClient"
import { ApiError, ErrorDetails } from "../util/error"
import { EmptyOcsResponse } from "../types"
...
export class DeckClient extends BaseApiClient {
    ...
    async removeLabelFromCard(boardId: number, stackId: number, cardId: number, labelId: number): Promise<void> {
        return await this.put<{ labelId: number }, void>(pathCardRemoveLabel(boardId, stackId, cardId), { labelId })
    }

    async assignUserToCard(boardId: number, stackId: number, cardId: number, userId: string): Promise<AssignUserResponse> {
        return await this.put<{ userId: string }, AssignUserResponse>(pathCardAssignUser(boardId, stackId, cardId), { userId })
    }

    async removeUserFromCard(boardId: number, stackId: number, cardId: number, userId: string): Promise<void> {
        return await this.put<{ userId: string }, void>(pathCardUnassignUser(boardId, stackId, cardId), { userId })
    }

    async reorderCard(boardId: number, stackId: number, cardId: number, payload: ReorderCardPayload): Promise<void> {
        return await this.put<{ order: number }, void>(pathCardReorder(boardId, stackId, cardId), payload)
    }
    ...
    async getUpcomingCards(): Promise<UpcomingCards> {
        return (await this.ocsGet<OcsUpcomingCardsRespone, null>(pathOcsUpcomingCards)).ocs.data
    }

    async queryCards(term: string, limit?: number): Promise<SearchedCard[]> {
        return (await this.ocsGet<OcsSearchCardsResponse, OcsSearchCardQuery>(pathOcsSearchCards, { term: term, limit: limit })).ocs.data
    }

    override async buildOcsErrorObject(request: RequestInfo, response: FetchResponse<EmptyOcsResponse>, options: ResolvedFetchOptions): Promise<ApiError> {
        // console.log("DeckClient.buildOcsErrorObject", request, response, options)
        // console.log(await response.text())
        return new DeckError({
            code: response.status,
            message: response._data?.ocs.meta.message ? response._data?.ocs.meta.message : '',
